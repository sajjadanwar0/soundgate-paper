#!/usr/bin/env python3
"""R1 -- analyze the sweep: leak rate per structural relation, with Wilson CIs.

The law the paper predicts (Sec. 3.3 mechanism): conc_same leaks, descendant
holds. ancestor/conc_earlier run before the pause exists (reported for
completeness, excluded from the B1 denominator). conc_later is the cell the
sweep charts empirically. Also reports the re-execution counts (gate node
body should run twice under FW-A's documented resume-from-node-start).

Usage: python3 analyze.py --results results_fwa.jsonl
"""
...
def main():
    ...
    a = ap.parse_args()
    buckets = defaultdict(lambda: [0, 0])      # relation -> [during_pause, total]
    dts = defaultdict(list)
    reexec = defaultdict(int)                   # executions_total -> count
